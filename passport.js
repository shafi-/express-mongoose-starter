...
const config = require('./configuration').passport;
...
passport.use(
  ...
  new FacebookTokenStrategy(
    {
      clientID: config.oauth.facebook.clientID || 'fb-client-id',
      clientSecret: config.oauth.facebook.clientSecret || 'fb-client-secret'
    },
    async (accessToken, refreshToken, profile, done) => {
      try {
        // console.log('profile', profile);
  ...
  new LocalStrategy(
    ...
    async (phone, password, done) => {
      try {
        ...
        if (!(passwordInDB && passport)) {
          const e = new Error('User password is not set');
          e.status = 400;
          done(e, false);
        }
